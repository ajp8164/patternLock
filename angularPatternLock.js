...
(function (root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        root.angularPatternLock = factory(root.angular);
  }
}(this, function (angular) {

return angular.module('angular-pattern-lock', [])
    .factory('patternLock', ['$document', '$window', function ($document, $window) {
        ...
        function readyDom(iObj) {
            ...
            html.push('</ul>');
            angular.element(angular.element(holder).html(html.join(''))).css({
                'width': (matrix[1] * (radius * 2 + margin * 2) + margin * 2) + 'px',
                'height': (matrix[0] * (radius * 2 + margin * 2) + margin * 2) + 'px'
            });

            //select pattern circle
            iObj.pattCircle = angular.element(iObj.holder[0].getElementsByClassName("patt-circ"));
        ...
        var startHandler = function(e, obj) {
            ...
            if (!iObj.option.patternVisible) {
                angular.element(iObj.holder).addClass('patt-hidden');
            }
            ...
            //assign events
            angular.element(this).on(touchMove, function(e) {
                moveHandler.call(this, e, obj);
            });
            iObj.touchMove = touchMove; //used to cancel the event listener

            angular.element(document).one(touchEnd, function() {
                endHandler.call(this, e, obj);
            });
            //set pattern offset
            var wrap = angular.element(iObj.holder[0].getElementsByClassName('patt-wrap')),
        ...
        var moveHandler = function(e, obj) {
            e.preventDefault();
            ...
            if (idx && ((option.allowRepeat && patternAry[patternAry.length - 1] !== pattId) || patternAry.indexOf(pattId) === -1)) {
                var elm = angular.element(li[idx - 1])[0];
                ...
                if (iObj.lastPosObj) {
                    ...
                    while (((iDiff === 0 && jDiff > 1) || (jDiff === 0 && iDiff > 1) || (jDiff == iDiff && jDiff > 1))) {
                        ...
                        if (option.allowRepeat || patternAry.indexOf(nextPattId) == -1) {
                            ...
                            //mark a point added
                            iObj.markPoint(angular.element(document.querySelector(li[nextPattId - 1]))[0], nextPattId);
        ...
        var endHandler = function(e, obj) {
            ...
            angular.element(iObj.holder).off(iObj.touchMove).removeClass('patt-hidden');
        ...
        InternalMethods.prototype = {
            ...
            markPoint: function(elm, pattId) {
                ...
                angular.element(elm).addClass('hovered');
            ...
            addLine: function(posObj) {
                ...
                if (patternAry.length > 1) {
                    ...
                    angular.element(_this.line).css({
                ...
                //to create new line
                var line = angular.element('<div class="patt-lines" style="top:' + (newY - 5) + 'px; left:' + (newX - 5) + 'px"></div>')[0];
                _this.line = line;
                _this.lineX1 = newX;
                _this.lineY1 = newY;
                //add on dom
            ...
            addDirectionClass: function(curPos) {
                ...
                if (direction) {
                    angular.element(line).addClass(direction + " dir");
                    angular.element(point).addClass(direction + " dir");
                }
            }
        };

        function PatternLock(plElem, option) {
            var self = this,
                token = self.token = Math.random(),
                iObj = objectHolder[token] = new InternalMethods(),
                holder = iObj.holder = angular.element(plElem);
            ...
            //remove undefined options, preserve options that are functions
            var optOnDraw = option.onDraw;
            option = JSON.parse(JSON.stringify(option));
            option.onDraw = optOnDraw;
            ...
            option = iObj.option = angular.extend({}, PatternLock.defaults, defaultsFixes, option);
            readyDom(iObj);

            //add class on holder
            angular.element(holder).addClass('patt-holder');

            //change offset property of holder if it does not have any property
            if (angular.element(holder).css('position') == "static") angular.element(holder).css('position', 'relative');

            //assign event
            angular.element(holder).on("touchstart mousedown", function(e) {
        ...
        PatternLock.prototype = {
            ...
            reset: function() {
                ...
                angular.element(iObj.holder[0].getElementsByClassName("patt-lines")).remove();
                ...
                angular.element(iObj.holder).removeClass('patt-error');

            },
            //to display error if pattern is not drawn correct
            error: function() {
                angular.element(objectHolder[this.token].holder).addClass('patt-error');
            },
    ...
    .directive('patternLock', ['patternLock', function (patternLock) {
        return {
            restrict: 'E',
            scope: {
                onInit: '&',
                ...
                lineOnMove: '=lineOnMove',
                enableSetPattern: '=enableSetPattern',
                delimiter: '=delimiter',
                allowRepeat: '=allowRepeat'
            },
            replace: true,
            template: '<div id="lockPattern"></div>',
            link: function (scope, element) {
                ...
                var lock = new patternLock(element, {
                    ...
                    margin: scope.margin,
                    radius: scope.radius,
                    patternVisible: scope.patternVisible,
                    lineOnMove: scope.lineOnMove,
                    enableSetPattern: scope.enableSetPattern,
                    delimiter: scope.delimiter,
                    allowRepeat: scope.allowRepeat
                });
                scope.onInit({
                    lock: lock
                });
